	...
	<body style="text-align: center" onload="noteAlert()">
		...
		<table class="table">
			<thead>
				...
				<c:forEach var="list" items="${news2List}" varStatus="status">
					<tr>
						<td>
							${status.count}
						</td>
						...
						<td>
							<a href="<%=path%>/News2Servlet?action=edit&id=${list.ID}">修改</a>&nbsp;|&nbsp;
							<a href="<%=path%>/News2Servlet?action=delete&id=${list.ID}">删除</a>
						</td>
					</tr>
				</c:forEach>
			</thead>
